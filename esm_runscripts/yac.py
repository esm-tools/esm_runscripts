class yac:
    def __init__(self, nb_of_couplings = 1, coupled_execs = ["echam", "fesom"], grids = ["atmo", "feom"], runtime = 1, debug_level=1):
        ...
        self.namcouple += ['\t<redirect redirect_of_root="false" redirect_stdout="false"/>']
        ...
        for exe in coupled_execs:
            ...
            self.namcouple += ['\t\t\t<name>'+str(exe)+'</name>']
            self.namcouple += ['\t\t\t<model>'+str(exe)+'</model>']
            print('exe: ', exe)
            if exe == 'echam6':
                self.namcouple += ['\t\t\t<simulated>atmosphere</simulated>']
            elif exe == 'fesom':
                self.namcouple += ['\t\t\t<simulated>ocean</simulated>']
        ...
        for field in range(nb_of_couplings):
            self.namcouple += ['\t\t<transient id="'+str(field+1)+'"  transient_standard_name="??transient_name'+str(field+1)+'??"/>']
        self.namcouple += ['\t</transients>']
        ...
        self.namcouple += ['\t\t\t<start_date>2000-01-01T00:00:00</start_date>']
        self.namcouple += ['\t\t\t<end_date>2000-03-01T00:00:00</end_date>']
        self.namcouple += ['\t\t\t<calendar>proleptic-gregorian</calendar>']
        self.namcouple += ['\t</dates>']
        self.namcouple += ['\t<timestep_unit>second</timestep_unit>']
        self.namcouple += ["\t<couples>"]
        self.namcouple += ['\t\t<couple>']
        self.namcouple += ['\t\t\t<component1 component_id="1" />']
        self.namcouple += ['\t\t\t<component2 component_id="2" />']
        self.next_coupling=1
        self.name = "yac"

    def add_coupling(self, direction, config):
        import sys

  #      self.namcouple += ['\t\t<couple>']
  #      self.namcouple += ['\t\t\t<component1 component_id="'+str(config["coupling_directions"][direction]["source"]["component_id"])+'" />']
  #      self.namcouple += ['\t\t\t<component2 component_id="'+str(config["coupling_directions"][direction]["target"]["component_id"])+'" />']


    def add_coupling_field(self, field, transient_id, direction, config):
        import sys
        ...
        print('namcouple: ', self.namcouple[index])

        self.namcouple += ['\t\t\t\t<transient_couple transient_id="'+str(transient_id)+'">']
#        self.namcouple += ['\t\t\t\t\t<source component_ref="'+str(config["coupling_directions"][direction]["source"]["component_id"])+'" transient_grid_ref="'+str(config["coupling_directions"][direction]["source"]["transient_grid_ref"])+'" />']
...
#        self.namcouple += ['\t\t\t\t\t<target transient_grid_ref="'+str(config["coupling_directions"][direction]["target"]["transient_grid_ref"])+'" />']
