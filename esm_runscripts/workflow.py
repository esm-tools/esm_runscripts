...
#
    ...
    if config["general"]["jobtype"] == "unknown":
        config["general"]["command_line_config"]["jobtype"] = config["general"]["workflow"]["first_task_in_queue"]
        config["general"]["jobtype"] = config["general"]["workflow"]["first_task_in_queue"]
    ...
    for subjob_cluster in gw_config["subjob_clusters"]:
        ...
        if "run_after" in gw_config["subjob_clusters"][subjob_cluster]:
            if "run_before" in gw_config["subjob_clusters"][subjob_cluster]:
                print(f"Specifying both run_after and run_before for cluster {subjob_cluster} may lead to problems.")
                print(f"Please choose.")
                sys.exit(-1)
            if not gw_config["subjob_clusters"][subjob_cluster]["run_after"] in gw_config["subjob_clusters"]:
                print(f"Unknown cluster {subjob_cluster['run_after']}.")
                sys.exit(-1)

            calling_cluster = gw_config["subjob_clusters"][subjob_cluster]["run_after"]
                
            if not subjob_cluster in gw_config["subjob_clusters"][calling_cluster]["next_submit"]:
                gw_config["subjob_clusters"][calling_cluster]["next_submit"].append(subjob_cluster)
            gw_config["subjob_clusters"][subjob_cluster]["called_from"] = calling_cluster
        ...
        if "run_before" in gw_config["subjob_clusters"][subjob_cluster]:
            ...
            if not called_cluster in gw_config["subjob_clusters"][subjob_cluster]["next_submit"]:
                gw_config["subjob_clusters"][subjob_cluster]["next_submit"].append(called_cluster)
...
def merge_single_entry_if_possible(entry, sourceconf, targetconf):
    if entry in sourceconf:
        if entry in targetconf and \
                not sourceconf[entry] == targetconf[entry]:
        ...
        if "workflow" in config[model]:
            ...
            if "subjobs" in w_config:
                ...
                for subjob in list(copy.deepcopy(w_config["subjobs"])):
                    ...
                    gw_config["subjobs"][subjob + "_" + model] = copy.deepcopy(w_config["subjobs"][subjob])
