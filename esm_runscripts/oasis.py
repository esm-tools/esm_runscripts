...
class oasis:
    def __init__(self, nb_of_couplings = 1, coupled_execs = ["echam", "fesom"], runtime = 1, debug_level=1,
        ...
        # PG: Fixup version to be tuples:
            ...
            print("Init of Oasis needs the argument mct_version to be either a tuple or a string!", flush=True)
            sys.exit(1)
    ...
    def add_coupling(self, lefts, lgrid, rights, rgrid, direction, transformation, restart_file, time_step, lresume, export_mode = "DEFAULT"):
        ...
        for pre in allpres:
            ...
            if pre.lower() == "checkin":
                ...
                if not coefficient:
                    print ("xmult needs to be defined for preprocessing BLASOLD", flush=True)
                    sys.exit(2)
                add_scalar = transformation["preprocessing"][pre].get("add_scalar", None)
                if not add_scalar:
                    print ("add_scalar needs to be defined (0 or 1) for preprocessing BLASOLD", flush=True)
                    sys.exit(2)
                ...
                if str(add_scalar) == "1":
                    ...
                    if not add_scalar:
                        print ("scalar_to_add needs to be defined if add_scalar is set to  1 for preprocessing BLASOLD", flush=True)
                        sys.exit(2)
        ...
        for thistrans in alltrans:
            ...
            if "mapping" == trans.lower():
                ...
                if not mapname:
                    print ("mapname needs to be defined for transformation MAPPING", flush=True)
                    sys.exit(2)
                ...
                if trans.lower() == "gauswgt":
                    ...
                    if not weight:
                        print ("weight needs to be defined for transformation GAUSWGT", flush=True)
                        sys.exit(2)
                    detail_line += " " + str(weight)
                if trans.lower() == "conserv":
                    normalization = transform.get("normalization", None)
                    if not normalization:
                        print ("normalization (FRACAREA, DESTAREA or FRACNNEI) needs to be defined for transformations CONSERV", flush=True)
                        sys.exit(2)
                    order = transform.get("order", None)
                    if not order:
                        print ("order (FIRST or SECOND) needs to be defined for transformation CONSERV", flush=True)
                        sys.exit(2)
        ...
        for post in allpost:
            ...
            if post.lower() == "conserv":
                ...
                if not method:
                    print (" a method (GLOBAL, GLBPOS, BASBAL or BASPOS) needs to be defined for postprocessing CONSERV", flush=True)
                    sys.exit(2)
                ...
                if not coefficient:
                    print ("xmult needs to be defined for postprocessing BLASNEW", flush=True)
                    sys.exit(2)
                ...
                if str(add_scalar) == "1":
                    ...
                    if not add_scalar:
                        print ("scalar_to_add needs to be defined if add_scalar is set to  1 for postprocessing BLASNEW", flush=True)
                        sys.exit(2)
                    detail_line = " CONSTANT" + str(add_scalar)
                    trafo_details.append(detail_line.strip())
    ...
    def prepare_restarts(self, restart_file, all_fields, models, config):
        ...
        for field, model, exe in zip(all_fields, models, exes):
            ...
            for thisfile in thesefiles:
                ...
                print("cdo -O seltimestep," + str(lasttimestep) + " " + thisfile + " onlyonetimestep.nc", flush=True)
                os.system("cdo -O seltimestep," + str(lasttimestep) + " " + thisfile + " onlyonetimestep.nc")
                print("ncwa -O -a time onlyonetimestep.nc notimestep_" + field + ".nc", flush=True)
